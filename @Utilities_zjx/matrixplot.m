...
[FigShape,FigSize,FigStyle,FillStyle,DisplayOpt,TextColor,XVarNames,...
    YVarNames,ColorBar,GridOpt,YTextRoation] = parseInputs(varargin{:});
...
% 设置X轴和Y轴刻度标签的缩进方式
MyTickLabel(gca,FigStyle,YTextRoation);
...
%  调整坐标轴刻度标签子函数
% ---------------------------------------------------
function MyTickLabel(ha,tag,YTextRoation)
...
function [FigShape,FigSize,FigStyle,FillStyle,DisplayOpt,TextColor,...
    XVarNames,YVarNames,ColorBar,GridOpt,YTextRoation] = parseInputs(varargin)

if mod(nargin,2)~=0
    error('输入参数个数不对，应为成对出现');
end
pnames = {'FigShape','FigSize','FigStyle','FillStyle','DisplayOpt',...
    'TextColor','XVarNames','YVarNames','ColorBar','Grid','YTextRoation'};
dflts =  {'Square','Full','Auto','Fill','On','k','','','Off','On',90};
[FigShape,FigSize,FigStyle,FillStyle,DisplayOpt,TextColor,XVarNames,...
    YVarNames,ColorBar,GridOpt,YTextRoation] = parseArgs(pnames, dflts, varargin{:});
...
validateattributes(YTextRoation,{'numeric'},{'nonempty'},mfilename,'YTextRoation');

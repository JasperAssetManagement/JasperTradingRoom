function [dNav,dpos] = NSW(dRtns,dObsArg,dEMAarg,dPercentArg,bForget)
...
for iDate = dObsArg+1:nDates - 2
    ...
    if any(bSigLong|bSigShort)
        ...
        dpos(iDate+2,bActLong) = 1/max(nActLong,1)/2;

        bActShort = bSigShort;
        nActShort = sum(bActShort,2);
        dpos(iDate+2,bActShort) = -1/max(nActShort,1)/2;
    end
end

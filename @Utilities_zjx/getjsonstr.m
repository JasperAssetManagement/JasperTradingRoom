function jsonstr=getjsonstr(rootname,obj,varargin)
%
...
jsonstr=obj2json(rootname,obj,rootlevel,opt);
if(rootisarray)
    jsonstr=sprintf('%s%s',jsonstr,nl);
else
    jsonstr=sprintf('{%s%s%s}\n',nl,jsonstr,nl);
end

jsonp=jsonopt('JSONP','',opt);
if(~isempty(jsonp))
    jsonstr=sprintf('%s(%s);%s',jsonp,jsonstr,nl);
end
...
for i=1:dim(1)
    if(dim(1)>1)
        txt(end+1:end+2)={padding2,'['};
    end
    for j=1:dim(2)
       if isa(item{i,j},'char')
           txt{end+1}=obj2json(name,item{i,j},0,varargin{:});
       else

function [insertMask, returnedKeys] = upsert(conn,tableName,fieldNames,keyFields,data, varargin)
...
%   Author: Sven Holcombe, 2015-09-01

% Handle user configuration input
IP = inputParser;
% IP.addParameter('updateFcn', @update,     @(x)isa(x,'function_handle'));
% IP.addParameter('insertFcn', @fastinsert, @(x)isa(x,'function_handle'));
% IP.addParameter('dateFields', []);
% IP.addParameter('returnKeys',false,@(x)iscellstr(x)||(isscalar(x)&&nnz(x==[0 1])==1));
% IP.addParameter('debug', false);
addParamValue(IP, 'updateFcn', @update,     @(x)isa(x,'function_handle'));
addParamValue(IP, 'insertFcn', @fastinsert, @(x)isa(x,'function_handle'));
addParamValue(IP, 'dateFields', []);
addParamValue(IP, 'returnKeys',false,@(x)iscellstr(x)||(isscalar(x)&&nnz(x==[0 1])==1));
addParamValue(IP, 'debug', false);
IP.parse(varargin{:});
doPrint = IP.Results.debug;
updateFcn = IP.Results.updateFcn;
insertFcn = IP.Results.insertFcn;
doReturnKeys = iscellstr(IP.Results.returnKeys) || IP.Results.returnKeys;
if ~doReturnKeys && nargout>1
    warning('upsert:noKeys', 'Second output (returned keys) will be empty because ''returnKeys'' option was not set')
end
...
for i=1:length(keyFields)
    if keyFieldsIsnumeric(i)
        ...
        if all(inSet==round(inSet))
            inStr = sprintf('%d,',inSet);
        else
...
if ~isempty(fetchedData)
    ...
    for i = 1:length(keyFields)
        if keyFieldsIsnumeric(i)
            ...
            thisFetchedData = cast(cell2mat(fetchedData(:,i)), 'like',thisUpsertData);
...
if any(insertMask)
    ...
    if doReturnKeys
        insertedRK = insertFcn(conn,tableName,fieldNames,data(insertMask,:),'returnKeys',IP.Results.returnKeys);
    else
...
if doReturnKeys
    updatedRK = updateFcn(conn,tableName,fieldNames(otherFields),data(~insertMask,otherFields), dataWhereClauses(~insertMask),'returnKeys',IP.Results.returnKeys);
    returnedKeys = zeros(size(cat(1,insertedRK, updatedRK)));
    if any(insertMask)
        returnedKeys(insertMask,:) = insertedRK;
    end
    if any(~insertMask)
        returnedKeys(~insertMask,:) = updatedRK;
    end
else
